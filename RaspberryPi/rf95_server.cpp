			...
			if (bcm2835_gpio_eds(RF_IRQ_PIN)) {
				...
				if (rf95.available()) {
					...
					if (rf95.recv(buf, &len)) {
						...
						memcpy(&count, buf, sizeof(long));
						memcpy(&tempf, buf + 4, sizeof(float));
						memcpy(&co2, buf + 8, sizeof(float));
						memcpy(&ozone, buf + 16, sizeof(float));
						memcpy(&ppm, buf + 20, sizeof(float));
						memcpy(&battery, buf + 24, sizeof(float));
						printf("SIZEOF FLOAT: %d\n", sizeof(float));
						printf("Packet[%02d] #%d => #%d %ddB: '%d'\n", len, from, to, rssi, count);
						printf("Temp: %f\n", tempf);
						printf("CO2: %f\n", co2);
						printf("OZone: %f\n", ozone);
						printf("PPM: %f\n", ppm);
						printf("Battery: %f\n", battery);
						hexDump(NULL, buf, sizeof(buf));
